...
def upload():
    ...
    if not persona or not title or not description:
        return "Faltan datos"
    ...
    (id, persona, title, description, image, qr)
    VALUES (?, ?, ?, ?, ?, ?)
""", (
    uid,
    persona,
...
def persona(nombre):
    ...
    recuerdos = conn.execute(
        "SELECT * FROM experiences WHERE persona=?",
        (nombre,)
    ).fetchall()
...
if contexto:
    ...
    respuesta = f"""
Memoria encontrada para {nombre}
...
"""
    ...
    respuesta = f"No encontré recuerdos asociados a {nombre}."
