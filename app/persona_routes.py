...
from app.character.conversation import (
    crear_conversacion,
...
def api_persona_experiencias(persona_id):
    ...
    try:
        response = (
            current_app.supabase.table("experiences")
...
def api_conversation(conversation_id):
    ...
    conversation = obtener_conversacion(conversation_id)
    if not conversation:
        return jsonify({"error": "Conversación no encontrada"}), 404
    return jsonify({**conversation, "mensajes": obtener_mensajes(conversation_id)})
...
def api_persona_chat(persona_id):
    ...
    if conversation_id:
        ...
        conversation = crear_conversacion(persona_id, str(uuid.uuid4()), metadata={"channel": "qr_web"})
        if not conversation:
