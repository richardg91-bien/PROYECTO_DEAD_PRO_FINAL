  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-amber-50 via-white to-amber-50/30 flex flex-col">
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-amber-200 px-4 py-3">
        <div className="max-w-2xl mx-auto flex items-center gap-3">
          ...
          {persona.foto_principal ? (
            <img src={persona.foto_principal} alt={persona.nombre} className="w-11 h-11 rounded-full object-cover border-2 border-amber-400" />
          ) : (
            <div className="w-11 h-11 rounded-full border-2 border-amber-400 flex items-center justify-center bg-amber-50 text-amber-800 font-bold">{persona.nombre?.charAt(0)}</div>
          )}
        ...
        <div className="max-w-2xl mx-auto space-y-4">
          {historial.length === 0 && (
            <section className="text-center py-12">
              ...
              <h2 className="font-serif text-2xl font-bold text-gray-800 mb-2">{persona.nombre}</h2>
              {persona.bio && <p className="text-gray-500 max-w-lg mx-auto mb-5">{persona.bio}</p>}
              <p className="text-sm text-gray-400">Podés iniciar una conversación con este memorial.</p>
            </section>
          )}
          {historial.map((m, i) => (
            <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[78%] rounded-2xl px-4 py-3 text-sm leading-relaxed shadow-sm ${m.role === "user" ? "bg-amber-600 text-white rounded-br-sm" : "bg-white border border-gray-100 text-gray-800 rounded-bl-sm"}`}>
                {m.content}
              </div>
            </div>
          ))}
      ...
      <div className="sticky bottom-0 bg-white/95 backdrop-blur border-t px-4 py-3">
        <form onSubmit={enviar} className="max-w-2xl mx-auto flex gap-2">
          <textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); enviar(e); } }} rows={1} disabled={enviando} placeholder={`Escribile a ${persona.nombre}…`} className="flex-1 resize-none px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:outline-none focus:border-amber-400" />
          <button type="submit" disabled={!input.trim() || enviando} className="w-11 h-11 rounded-full bg-amber-600 text-white shadow disabled:opacity-40">➤</button>
        </form>
      </div>
    </div>
  );
