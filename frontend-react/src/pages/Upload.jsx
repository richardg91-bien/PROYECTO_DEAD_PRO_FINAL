  ...
  useEffect(() => {
    api.get("/api/personas")
      .then(res => {
        const rows = res.data || [];
        setPersonas(rows);
        if (rows.length === 1) setPersonaId(rows[0].id);
      })
      ...
      <main className="max-w-xl mx-auto px-6 py-8">
        ...
        {personas.length === 0 ? (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 text-center">
            <h2 className="font-serif font-bold text-gray-800 mb-2">Primero creá una persona</h2>
            <p className="text-sm text-gray-500 mb-5">El recuerdo debe quedar vinculado a una identidad PERSONA.</p>
            <button
          ...
          <form onSubmit={handleSubmit} className="space-y-4 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <div>
              ...
              <select
                ...
                required
                className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm text-gray-800 bg-white focus:outline-none focus:border-[#D4AF37] focus:ring-2 focus:ring-[#D4AF37]/20"
              >
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:border-[#D4AF37] focus:ring-2 focus:ring-[#D4AF37]/20"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:border-[#D4AF37] focus:ring-2 focus:ring-[#D4AF37]/20 resize-none"
              />
            </div>
